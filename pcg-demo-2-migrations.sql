-- ============================================
-- PCG DEMO: PART 2 of 2 - All Migrations
-- Run AFTER Part 1 completes
-- ============================================

-- Drop all views upfront so migrations can recreate them without column-count conflicts
DROP VIEW IF EXISTS public.en_workflows_list_view;
DROP VIEW IF EXISTS public.enprotec_stock_receipts_list_view;
...
select
    ...
    (
        ...
        from (
            ...
            from public.enprotec_workflow_items wi
            join public.enprotec_stock_items si on wi.stock_item_id = si.id
            left join public.enprotec_inventory inv
...
ALTER TABLE public.enprotec_salvage_requests
...
ALTER TABLE public.enprotec_workflow_requests
    ADD COLUMN IF NOT EXISTS driver_name text,
    ADD COLUMN IF NOT EXISTS vehicle_registration text;

-- Create attachments table for workflows
CREATE TABLE IF NOT EXISTS public.enprotec_workflow_attachments (
...
CREATE INDEX IF NOT EXISTS enprotec_workflow_attachments_request_idx
    ON public.enprotec_workflow_attachments (workflow_request_id);
...
BEGIN
    ...
    INSERT INTO public.enprotec_stock_receipts (
        stock_item_id,
        quantity_received,
        received_by_id,
        store,
    ...
    FROM public.enprotec_inventory
    ...
    ELSE
        -- Insert new
        INSERT INTO public.enprotec_inventory (
            stock_item_id,
            store,
            quantity_on_hand,
            location
    ...
    IF p_is_return AND p_return_workflow_id IS NOT NULL THEN
        UPDATE public.enprotec_workflow_requests
        SET 
...
BEGIN
    -- 1. Create Request
    INSERT INTO public.enprotec_workflow_requests (
        request_number,
        type,
        requester_id,
        site_id,
    ...
    LOOP
        INSERT INTO public.enprotec_workflow_items (
            workflow_request_id,
            stock_item_id,
            quantity_requested
        ) VALUES (
    ...
    IF p_comment IS NOT NULL AND length(p_comment) > 0 THEN
        INSERT INTO public.enprotec_workflow_comments (
            workflow_request_id,
            user_id,
            comment_text
        ) VALUES (
...
BEGIN
    ...
    -- Get the user's role and assigned sites from enprotec_users table
    SELECT role, sites INTO v_user_role, v_user_sites
    FROM public.enprotec_users
...
-- Migration: Recreate enprotec_users.departments as TEXT[] for Dynamic Stores
-- Date: 2026-01-20
-- Description: Recreates the departments column as TEXT[] (was dropped by CASCADE in migration 2)
--              This allows assignment of dynamic department values from enprotec_departments table.
...
ALTER TABLE public.enprotec_users
...
ON public.enprotec_users USING GIN (departments);

-- Add comment explaining the change
COMMENT ON COLUMN public.enprotec_users.departments IS 'Array of store/department codes from enprotec_departments table. Changed from department[] ENUM to TEXT[] to support dynamic department management.';
...
-- Date: 2026-01-20
-- Description: Creates enprotec_departments table to replace hardcoded Store enum values
...
CREATE TABLE IF NOT EXISTS public.enprotec_departments (
...
CREATE INDEX IF NOT EXISTS idx_en_departments_name ON public.enprotec_departments(name);

-- Enable Row Level Security
ALTER TABLE public.enprotec_departments ENABLE ROW LEVEL SECURITY;

-- Drop existing policies if they exist (for idempotency)
DROP POLICY IF EXISTS "Anyone can view departments" ON public.enprotec_departments;
DROP POLICY IF EXISTS "Admins can insert departments" ON public.enprotec_departments;
DROP POLICY IF EXISTS "Admins can update departments" ON public.enprotec_departments;
DROP POLICY IF EXISTS "Admins can delete departments" ON public.enprotec_departments;

-- RLS Policy: Anyone can view departments
CREATE POLICY "Anyone can view departments" ON public.enprotec_departments
    FOR SELECT USING (true);

-- RLS Policy: Only admins can insert departments
CREATE POLICY "Admins can insert departments" ON public.enprotec_departments
    FOR INSERT WITH CHECK (
        EXISTS (
            SELECT 1 FROM public.enprotec_users
            WHERE id = auth.uid() AND role = 'Admin' AND status = 'Active'
        )
    );

-- RLS Policy: Only admins can update departments
CREATE POLICY "Admins can update departments" ON public.enprotec_departments
    FOR UPDATE USING (
        EXISTS (
            SELECT 1 FROM public.enprotec_users
            WHERE id = auth.uid() AND role = 'Admin' AND status = 'Active'
...
CREATE POLICY "Admins can delete departments" ON public.enprotec_departments
    FOR DELETE USING (
        EXISTS (
            SELECT 1 FROM public.enprotec_users
            WHERE id = auth.uid() AND role = 'Admin' AND status = 'Active'
...
DROP TRIGGER IF EXISTS set_updated_at_en_departments ON public.enprotec_departments;
...
COMMENT ON TABLE public.enprotec_departments IS 'Stores/Departments table for dynamic management. Replaces hardcoded Store enum while maintaining backward compatibility via code field.';
COMMENT ON COLUMN public.enprotec_departments.code IS 'Short code matching Store enum values (e.g., OEM, Operations). Used for backward compatibility with existing workflow_requests.department values.';
COMMENT ON COLUMN public.enprotec_departments.status IS 'Active departments appear in dropdowns. Frozen departments are hidden but existing references remain valid.';
...
UPDATE public.enprotec_salvage_requests
...
DELETE FROM public.enprotec_departments
WHERE code IN (
    SELECT code
    FROM public.enprotec_departments
    ...
    AND (
        ...
        NOT EXISTS (SELECT 1 FROM public.enprotec_workflow_requests WHERE department = code)
        AND NOT EXISTS (SELECT 1 FROM public.enprotec_salvage_requests WHERE source_department = code)
        AND NOT EXISTS (SELECT 1 FROM public.enprotec_users WHERE code = ANY(departments))
    )
    AND (
...
ALTER TABLE public.enprotec_departments
...
DROP TRIGGER IF EXISTS prevent_core_department_code_change_trigger ON public.enprotec_departments;
...
COMMENT ON CONSTRAINT check_department_code_matches_store_type ON public.enprotec_departments IS
...
COMMENT ON TRIGGER prevent_core_department_code_change_trigger ON public.enprotec_departments IS
...
COMMENT ON TABLE public.enprotec_departments IS
...
ON public.enprotec_stock_items(part_number);
...
ON public.enprotec_inventory(store, quantity_on_hand);
...
ON public.enprotec_workflow_requests(department, current_status);

-- Index for priority filtering
CREATE INDEX IF NOT EXISTS idx_en_workflow_requests_priority
ON public.enprotec_workflow_requests(priority);
...
ON public.enprotec_workflow_comments(workflow_request_id);

-- Index for created_at (ordering)
CREATE INDEX IF NOT EXISTS idx_en_workflow_comments_created_at
ON public.enprotec_workflow_comments(created_at);
...
ANALYZE public.enprotec_stock_items;
ANALYZE public.enprotec_inventory;
ANALYZE public.enprotec_workflow_requests;
ANALYZE public.enprotec_workflow_items;
ANALYZE public.enprotec_users;
ANALYZE public.enprotec_sites;
ANALYZE public.enprotec_salvage_requests;
ANALYZE public.enprotec_stock_receipts;
ANALYZE public.enprotec_workflow_comments;
ANALYZE public.enprotec_departments;
...
-- Date: 2026-01-20
-- Description: Recreates enprotec_workflows_view and enprotec_salvage_requests_view after converting
...
BEGIN
    ...
    FROM public.enprotec_stock_items
    ...
    RETURN json_build_object(
        ...
        'receipt_id', v_receipt_id,
        'new_quantity', (SELECT quantity_on_hand FROM public.enprotec_inventory WHERE id = v_inventory_id)
    );
...
-- Items are stored in the enprotec_workflow_items table (NOT as JSONB in main table)
-- ============================================================================
...
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM public.enprotec_users WHERE id = p_requester_id AND status = 'Active') THEN
        RETURN json_build_object(
            'success', FALSE,
    ...
    -- Insert each item into enprotec_workflow_items table
    FOR v_item IN SELECT * FROM jsonb_array_elements(p_items)
    LOOP
        ...
        IF NOT EXISTS (SELECT 1 FROM public.enprotec_stock_items WHERE id = v_stock_item_id) THEN
            -- Rollback by deleting the workflow request we just created
            DELETE FROM public.enprotec_workflow_requests WHERE id = v_workflow_id;
...
-- Add comment
COMMENT ON FUNCTION public.process_stock_request IS 'Atomically processes stock request creation with items stored in enprotec_workflow_items table';

-- Add fleet_role column (required by app's userProfile query)
ALTER TABLE public.enprotec_users ADD COLUMN IF NOT EXISTS fleet_role TEXT;
...
-- 1. Admin User
INSERT INTO public.enprotec_users (
    id,
    name,
    email,
    password,
...
-- Add store column to enprotec_stock_receipts table
ALTER TABLE public.enprotec_stock_receipts
ADD COLUMN IF NOT EXISTS store TEXT;

-- Backfill store data from enprotec_inventory for existing receipts
UPDATE public.enprotec_stock_receipts sr
SET store = (
    SELECT inv.store
    FROM public.enprotec_inventory inv
...
DROP VIEW IF EXISTS public.enprotec_stock_receipts_view CASCADE;

CREATE VIEW public.enprotec_stock_receipts_view AS
SELECT
    sr.id,
    sr.stock_item_id AS "stockItemId",
    si.part_number AS "partNumber",
...
GRANT SELECT ON public.enprotec_stock_receipts_view TO authenticated;

-- Add index for better query performance when filtering by store
CREATE INDEX IF NOT EXISTS idx_en_stock_receipts_store ON public.enprotec_stock_receipts(store);
...
DROP VIEW IF EXISTS public.enprotec_workflows_view CASCADE;

CREATE VIEW public.enprotec_workflows_view AS
SELECT
    wr.id,
    wr.request_number AS "requestNumber",
    wr.type,
    ...
    (
        ...
        FROM public.enprotec_workflow_attachments a
...
GRANT SELECT ON public.enprotec_workflows_view TO authenticated;
...
DROP TRIGGER IF EXISTS on_dispatch_trigger ON public.enprotec_workflow_requests;
...
DROP TRIGGER IF EXISTS trg_inventory_stock_movement ON public.enprotec_inventory CASCADE;
DROP TRIGGER IF EXISTS trg_receipt_stock_movement ON public.enprotec_stock_receipts CASCADE;
DROP TRIGGER IF EXISTS update_stock_movement_on_inventory_change ON public.enprotec_inventory CASCADE;
...
BEGIN
    -- Validate stock item exists
    IF NOT EXISTS (SELECT 1 FROM public.enprotec_stock_items WHERE id = p_stock_item_id) THEN
        RETURN json_build_object(
            'success', FALSE,
            'error', 'Stock item not found'
        );
...
-- Step 1: Drop ALL existing policies on enprotec_workflow_requests
DO $$
DECLARE
    r RECORD;
BEGIN
    FOR r IN (SELECT policyname FROM pg_policies WHERE tablename = 'enprotec_workflow_requests' AND schemaname = 'public')
    LOOP
        EXECUTE 'DROP POLICY IF EXISTS "' || r.policyname || '" ON public.enprotec_workflow_requests';
...
ON public.enprotec_workflow_requests
FOR SELECT
...
ALTER TABLE public.enprotec_workflow_requests ENABLE ROW LEVEL SECURITY;
...
FROM pg_policies
WHERE tablename = 'enprotec_workflow_requests'
...
-- ============================================================================
-- Add store column to enprotec_stock_receipts and update view
-- ============================================================================
...
-- Step 1: Add store column to enprotec_stock_receipts table
ALTER TABLE public.enprotec_stock_receipts
ADD COLUMN IF NOT EXISTS store TEXT;

-- Step 2: Backfill store data from enprotec_inventory for existing receipts
...
-- Update comment
COMMENT ON VIEW public.enprotec_stock_receipts_view IS 'View for stock receipts with joined user, stock item, and store data';
...
ON public.enprotec_stock_items USING gin (part_number gin_trgm_ops);
...
ON public.enprotec_workflow_requests (current_status, created_at DESC);

CREATE INDEX IF NOT EXISTS idx_workflow_requests_department
ON public.enprotec_workflow_requests (department);
...
ON public.enprotec_workflow_requests (type);
...
-- ============================================================================
-- Stock Movements Indexes (skipped - table not used in this project)
-- ============================================================================
...
ON public.enprotec_stock_receipts (store);
...
ON public.enprotec_stock_receipts (delivery_note_po);
...
ON public.enprotec_workflow_comments (user_id);

CREATE INDEX IF NOT EXISTS idx_workflow_comments_created
ON public.enprotec_workflow_comments (created_at DESC);
...
ANALYZE public.enprotec_stock_items;
ANALYZE public.enprotec_inventory;
-- ANALYZE public.en_stock_movements; -- table not used in this project
ANALYZE public.enprotec_stock_receipts;
ANALYZE public.enprotec_workflow_requests;
ANALYZE public.enprotec_workflow_items;
ANALYZE public.enprotec_sites;
ANALYZE public.enprotec_users;
ANALYZE public.enprotec_workflow_comments;
ANALYZE public.enprotec_workflow_attachments;
ANALYZE public.enprotec_salvage_requests;
...
CREATE OR REPLACE VIEW public.enprotec_stock_receipts_list_view AS
SELECT
    sr.id,
    sr.stock_item_id AS "stockItemId",
    si.part_number AS "partNumber",
...
-- Grant permissions
GRANT SELECT ON public.enprotec_stock_receipts_list_view TO authenticated;

-- Add comment
COMMENT ON VIEW public.enprotec_stock_receipts_list_view IS 'Lightweight stock receipts view for list pages';
...
BEGIN
    ...
    RAISE NOTICE '- enprotec_stock_receipts_list_view (for Reports)';
    RAISE NOTICE '';
    RAISE NOTICE 'Usage:';
...
DROP POLICY IF EXISTS "authenticated_select_workflow_requests" ON public.enprotec_workflow_requests;
DROP POLICY IF EXISTS "authenticated_insert_workflow_requests" ON public.enprotec_workflow_requests;
DROP POLICY IF EXISTS "authenticated_update_workflow_requests" ON public.enprotec_workflow_requests;
DROP POLICY IF EXISTS "authenticated_delete_workflow_requests" ON public.enprotec_workflow_requests;
...
USING (
    ...
    (SELECT role FROM public.enprotec_users WHERE id = auth.uid()) = 'Admin'
    OR
...
USING (
    ...
    EXISTS (
        ...
        AND (
            ...
            (
                -- Check site access
                enprotec_workflow_requests.site_id = ANY(u.sites::uuid[])
                AND
                -- Check role matches workflow status
                (
                    -- Ops Manager can approve REQUEST_SUBMITTED
                    (enprotec_workflow_requests.current_status = 'Request Submitted' AND u.role = 'Operations Manager')
                    OR
                    -- Stock Controller can approve AWAITING_OPS_MANAGER
                    (enprotec_workflow_requests.current_status = 'Awaiting Ops Manager' AND u.role = 'Stock Controller')
                    OR
                    -- Equipment Manager can approve AWAITING_EQUIP_MANAGER
                    (enprotec_workflow_requests.current_status = 'Awaiting Equip. Manager' AND u.role = 'Equipment Manager')
                    OR
                    -- Stock Controller or Storeman can mark AWAITING_PICKING
                    (enprotec_workflow_requests.current_status = 'Awaiting Picking' AND u.role IN ('Stock Controller', 'Storeman'))
                    OR
                    -- Security or Driver can dispatch PICKED_AND_LOADED
                    (enprotec_workflow_requests.current_status = 'Picked & Loaded' AND u.role IN ('Security', 'Driver'))
                    OR
                    -- Driver or Site Manager can confirm EPOD for DISPATCHED
                    (enprotec_workflow_requests.current_status = 'Dispatched' AND u.role IN ('Driver', 'Site Manager'))
                    OR
                    -- Requester can always update their own request (for comments/attachments)
                    enprotec_workflow_requests.requester_id = auth.uid()
                )
            )
        )
    )
...
BEGIN
    ...
    FROM public.enprotec_workflow_requests
...
DROP POLICY IF EXISTS "role_site_based_update_workflow_requests" ON public.enprotec_workflow_requests;
...
USING (
    EXISTS (
        SELECT 1
        FROM public.enprotec_users u
        LEFT JOIN public.enprotec_sites s ON s.id = enprotec_workflow_requests.site_id
        WHERE u.id = auth.uid()
        AND (
            -- Admin can update everything
            u.role = 'Admin'
            ...
            (
                ...
                (
                    (enprotec_workflow_requests.current_status = 'Request Submitted' AND u.role = 'Operations Manager')
                    OR
                    (enprotec_workflow_requests.current_status = 'Awaiting Stock Controller' AND u.role = 'Stock Controller')
                    OR
                    (enprotec_workflow_requests.current_status = 'Awaiting Equip. Manager' AND u.role = 'Equipment Manager')
                    OR
                    (enprotec_workflow_requests.current_status = 'Awaiting Picking' AND u.role IN ('Stock Controller', 'Storeman'))
                    ...
                    (enprotec_workflow_requests.current_status = 'Dispatched' AND enprotec_workflow_requests.requester_id = auth.uid())
                )
            )
            OR
...
DROP POLICY IF EXISTS "site_based_select_workflow_requests" ON public.enprotec_workflow_requests;
DROP POLICY IF EXISTS "site_based_insert_workflow_requests" ON public.enprotec_workflow_requests;
DROP POLICY IF EXISTS "role_site_based_update_workflow_requests" ON public.enprotec_workflow_requests;
DROP POLICY IF EXISTS "admin_only_delete_workflow_requests" ON public.enprotec_workflow_requests;
...
USING (
    ...
    (
        EXISTS (
            SELECT 1
            FROM public.enprotec_users u
            JOIN public.enprotec_sites s ON s.id = enprotec_workflow_requests.site_id
            WHERE u.id = auth.uid()
...
-- Date: 2026-01-27
-- ============================================================================
-- The enprotec_users.sites array contains site NAMES (like "Kroondal"), not UUIDs
...
BEGIN
    ...
    RAISE NOTICE '- Joins enprotec_sites to get name from site_id';
